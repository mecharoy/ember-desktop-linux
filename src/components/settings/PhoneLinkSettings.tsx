      ...
      {info.enabled && (
        <>
          <p className="hint">
            ...
            hotspot instead. If a firewall asks (or blocks it), allow Elytra on the network: TCP 47821 and UDP 47820.
          </p>

          {info.pairingCode ? (
            <div className="fade-up flex flex-col items-start gap-1.5 rounded-xl border border-moss/40 bg-moss-wash/40 px-4 py-3.5">
              <span className="text-[13.5px] text-fg-dim">On your phone: Settings &rsaquo; Sync with computer &rsaquo; enter</span>
              <span className="select-text font-serif text-[32px] tracking-[0.12em] text-fg tabular-nums">{info.pairingCode}</span>
              <span className="hint">
          ...
          {info.peers.length > 0 && (
            <ul className="flex flex-col">
              {info.peers.map((p) => {
                ...
                return (
                  <li key={p.id} className="flex items-center gap-3 border-b border-line/70 py-2.5 last:border-b-0">
                    <span className="flex-1 text-[15px] text-fg">
                      {p.name || "Phone"}
                      <span className="block text-[12.5px] text-fg-faint">Last synced {sinceLabel(s?.last_sync_at ?? null)}</span>
                    </span>
