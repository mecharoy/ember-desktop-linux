# Elytra on Linux (Arch and others)

This folder is `elytra-desktop` with the changes Linux needs. Everything else is unchanged.
...
| Tray | Needs libayatana-appindicator. GNOME also needs the "AppIndicator and KStatusNotifierItem Support" extension. If no tray can be made, closing the window quits Elytra. |
...
| Backups | Go to `~/Documents/Elytra` (backups already in `~/Documents/Ember` still restore). Falls back to `$HOME/Documents` when the system has no Documents folder set (bare Arch). |
...
Bind a key to `ember-desktop --capture`. It toggles the capture bar in the running Elytra (or starts Elytra with only the bar).
...
On Wayland the bar's position is chosen by the compositor. Starting Elytra with `GDK_BACKEND=x11` (XWayland) restores exact placement.

## Phone sync

Open TCP 47821 and UDP 47820 on the firewall (ufw/firewalld/nftables) for the phone link. Ollama from the `ollama` package runs as a systemd service; Elytra uses it when it is already up.
